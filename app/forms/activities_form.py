...
class ActivityForm(FlaskForm):
    ...
    distance = FloatField("Distance", validators=[DataRequired(), NumberRange(min=0, max=1000000, message="Distance must be greater than 0.")])
    duration = IntegerField("Duration", validators=[DataRequired(), NumberRange(min=0, max=1000000, message="Duration must be greater than 0.")])
    calories = IntegerField("Calories", validators=[NumberRange(min=0, max=1000000, message="Calories must be greater than 0.")])
    elevation = IntegerField("Elevation", validators=[NumberRange(min=0, max=1000000, message="Elevation must be greater than 0.")])
